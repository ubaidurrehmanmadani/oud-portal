...
class AccountManagementController extends Controller
{
    ...
    public function update(Request $request, string $kind, int $id)
    {
        ...
        DB::transaction(function () use ($record, $kind, $data, $request) {
            $locked = $record->newQuery()->lockForUpdate()->findOrFail($record->id);
            $before = $locked->only(['name', 'email', 'role', 'department_id', 'can_submit_financial_reports', 'location', 'type', 'total_units', 'description']);
            $beforeAssignments = $kind === 'user' ? $locked->properties()->pluck('properties.id')->all() : ($kind === 'property' ? $locked->users()->pluck('users.id')->all() : []);
            if ($kind === 'user') {
                $locked->can_submit_financial_reports = $record->can_submit_financial_reports;
            }
            $record = $locked;
            ...
            AuditEvent::create([
                ...
                'event' => 'account.'.$kind.'.updated:'.$record->id,
                'changes' => ['before' => $before, 'after' => $record->only(array_keys($before)), 'assignments_before' => $beforeAssignments, 'assignments_after' => $kind === 'user' ? $record->properties()->pluck('properties.id')->all() : ($kind === 'property' ? $record->users()->pluck('users.id')->all() : [])],
