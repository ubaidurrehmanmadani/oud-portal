...
<section class="hero landlord-hero financial-home-hero"><div><p class="eyebrow">{{ __('financial.property_reports') }}</p><h1>{{ __('financial.title') }}</h1><p>{{ __('financial.intro') }}</p>@if($summaries->contains(fn ($summary) => data_get($summary->financial_data, 'reference')))<p class="reference-summary-note">{{ __('financial.reference_summary_note') }}</p>@endif</div></section>
<section class="property-summaries" aria-label="{{ __('workspace.properties') }}">
    @forelse ($properties as $property)
        ...
        @endphp
        <article class="card property-summary"><span class="card-kicker">{{ $latest?->report_month?->translatedFormat('F Y') ?? $latest?->period ?? __('financial.latest') }}{{ data_get($latest?->financial_data, 'reference') ? ' · Excel' : '' }}</span><h2>{{ $property->name }}</h2>
            <dl>
