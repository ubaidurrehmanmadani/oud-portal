...
<section class="card landlord-record-list property-list">
    @forelse ($properties->when($selectedProperty, fn ($rows) => $rows->where('id', $selectedProperty->id)) as $property)
        <article class="landlord-record"><span><strong>{{ $property->name }}</strong><small>{{ $property->type }} · {{ $property->location }} · {{ $property->total_units }} {{ __('workspace.units_short') }} · {{ $property->description }}</small></span><a class="row-action" href="{{ route('dashboard.landlord', ['property' => $property->id]) }}">{{ __('workspace.view') }}</a></article>
