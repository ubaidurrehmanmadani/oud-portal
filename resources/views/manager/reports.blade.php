...
<form method="GET" class="workspace-filters">
    ...
    <div class="field"><label for="status">{{ __('workspace.status') }}</label><select id="status" name="status"><option value="">{{ __('portal.manager_all_statuses') }}</option>@foreach (['draft', 'pending', 'returned', 'rejected', 'approved'] as $status)<option value="{{ $status }}" @selected(request('status') === $status)>{{ __('review.'.$status) }}</option>@endforeach</select></div>
...
@forelse ($submissions as $submission)
    <tr><td>{{ $submission->title }}<br><small>{{ $submission->file_name }}</small></td><td>{{ $submission->property->name }}</td><td>{{ $submission->report_month->format('Y-m') }}</td><td><span class="status">{{ __('review.'.$submission->status) }}</span>@if ($submission->submitted_at)<br><small>{{ $submission->submitted_at->format('Y-m-d H:i') }}</small>@endif</td><td><a href="{{ route('manager.reports.edit', $submission) }}">{{ $submission->status === 'draft' ? __('workspace.edit') : __('workspace.open') }}</a> · <a href="{{ route('manager.reports.download', $submission) }}">{{ __('workspace.download') }}</a></td></tr>
@empty
