...
17 September continuation: deployment seeding now prepares the documented Admin/Manager test teams and restricted Landlord, with transactional seed checkpoints that preserve later access changes. See README for initial credentials and migration order. Training publishing now dispatches the same queued publication notification used by documents, including after required private-file processing; current department and approval/suspension checks still gate recipients. Both English/Arabic manual sources document training notification behavior.
